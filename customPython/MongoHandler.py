...
def checkPAMinMongo(pamGenomicStart, pamSeq, gRNACollection):
	...
	return gRNACollection.find({"pamGenomicStart": pamGenomicStart, "pamSeq": pamSeq}).count()
...
def updategRNAinMongo(guideDict, gRNACollection):
        ...
        updates = {'Notes': guideDict['Notes'], 'label': guideDict['label'], 'guideScore': guideDict['guideScore'], 'otDesc': guideDict['otDesc']}
        ResultObject = gRNACollection.update_one({"pamGenomicStart": guideDict['pamGenomicStart'], "pamSeq": guideDict['pamSeq']}, {'$set': updates})
...
def writePrimerGFF(dbConnection, org):
	...
	for primerRecord in primerCollection.find({'status' : 'Accepted'}):
		...
		colNine += ';tm=' + str(primerRecord['leftTM']) + ';gc=' + str(primerRecord['leftGC']) + ';length=' + str(primerRecord['leftlen'])
...
def modifyDatabase(action, guideDict, geneCollection, gRNACollection):
	...
	if genomicSearchStart == -1 :
		sys.exit('Error! Input genomic coordinates are not in the correct format. Please contact CCM!')
	...
	if 'guideStart' in guideDict:
		guideDict['guideGenomicStart'] = genomicSearchStart+int(guideDict['guideStart'])+1
...
def addGenomicLocation(org, guideDict):
	...
	if len(hits) > 1:
		sys.exit("Guide sequence " + str(searchSeq) + " has " + str(len(hits)) + " identical matches in the genome")
	if len(hits) < 1:
	...
	try:
			...
			if action == "insert":
				...
				for field in essentialFields:
					if field not in guideDict:
						print("Field "+ str(field)+ " doesn't exist in the request. Aborting!")
						return
		...
		try:
			...
			if len(cli_arguments) > 2:
				print("Unexpected number of arguments, only the organism code (i.e. mm10) should be passed")
				return
			elif len(cli_arguments) == 1:
				print("To rewrite the gff files, provide the organism name (i.e. mm10) to the script")
				return
			else:
				org = cli_arguments[1]
				print("Will attempt to rewrite primer and guide GFF files for " + str(org))
				writeGFF(org)
