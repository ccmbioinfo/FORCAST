...
def load_geneinfo_RGENs(
    ...
    if mongo_username is not None and mongo_password is not None:
        mongo_uri = "mongodb://%s:%s@%s" % (
            ...
            "localhost",
        )  # straight up copied from api.mongodb.com
    ...
    if int(ensembl_version) <= int(curr_release):
        print(
            f"Metadata for newer Ensembl release ({curr_release}) already exists in Mongo database"
        )
        return True
        ...
        for line in inp_fh:
            if line.startswith("#") is False:
                ...
                if "gene" in tmpArr[2].lower():
                    tmpDict = dict(
                        [
                            [val for val in column.split("=")]
                            for column in tmpArr[8].split(";")
                        ]
                    )
...
if __name__ == "__main__":
    if len(sys.argv) > 4:
        ...
        print(
            f"Usage: {sys.argv[0]} <genes.gff3> <Ensembl version> <species> <assembly>",
            file=sys.stderr,
        )
        exit(1)
