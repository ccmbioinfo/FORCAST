...
def getRgenRecord(rgenID, dbConnection):
    ...
    if rgenRecord.count() == 1:
        ...
        raise Exception(
            "Invalid number of records returned from RGEN database for rgenID "
            + str(rgenID)
        )
        return
    ...
    for line in out.splitlines():
        ...
        for offTarget in guideDict[guideID]["offtargets"]:
            if offTarget["loc"] == location:
                ...
                offTarget["context"] = (
                    formatContext("", contextString)
                    if "context" not in offTarget
                    else formatContext(offTarget["context"], contextString)
                )
...
def formatContext(existingContext, newIntersection):
    ...
    if newIntersection == "exonic":
        ...
        if "intronic" in existingContext:
            ...
            return existingContext.replace(
                "exonic", "exonic/intronic"
            )  # give more specific categorization
        else:
            return newIntersection + " | " + existingContext
