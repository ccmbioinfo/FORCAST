...
def error_catching(seq, cutsite):
    ...
    if cutsite < 1 or cutsite > len(seq) - 1:
        return (
            True,
            "Cutsite index is not within the sequence. Cutsite must be an integer between index 1 and len(seq) - 1, inclusive.",
        )
...
def __predict_ins(seq, cutsite, pred_del_df, total_phi_score):
    ...
    onebp_features = (
        fiveohmapper[fivebase]
        + threeohmapper[threebase]
        + [precision]
        + [log_phi_score]
    )
...
def __build_stats(seq, cutsite, pred_df, total_phi_score):
    # Precision stats
    overall_precision = 1 - entropy(pred_df["Predicted frequency"]) / np.log(
        len(pred_df)
    )
...
def predict(seq, cutsite):
    ...
    pred_df = __predict_ins(seq, cutsite, pred_del_df, total_phi_score)
...
def add_mhless_genotypes(pred_df, stats, length_cutoff=None):
    ...
    for del_len in range(max_del_len):
        crit = (
            ...
            & (pred_df["Length"] == del_len)
            & (pred_df["Genotype position"] == "e")
        )
        subset = pred_df[crit]
        if len(subset) == 0:
            continue
...
# Init
##
def init_model(run_iter="aax", param_iter="aag", celltype="mESC"):
    global init_flag
    if init_flag != False:
        return
