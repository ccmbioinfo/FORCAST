...
class GuideSearch:
    def __init__(self, **kwargs):
        ...
        if self.action in ["initialize", "update_input"]:
            ...
            if "searchInput" not in kwargs:
                ...
                self.sendErrorHTML(
                    "'searchInput', " + str(kwargs["searchInput"]) + " is not valid"
                )
            else:
    ...
    def inputRowHTML(self):
        ...
        template_path = os.path.join(
            ...
            "src/guide-finder/jinja2-templates/input_region.html",
        )
    ...
    def initialHTML(self):
        ...
        template_path = os.path.join(
            self.dbConnection.ROOT_PATH, "src/guide-finder/jinja2-templates/select.html"
        )
    ...
    def sendErrorHTML(self, errorString):
        ...
        template_path = os.path.join(
            self.dbConnection.ROOT_PATH, "src/guide-finder/jinja2-templates/error.html"
        )
    ...
    def getOverlappingGenesHTML(self):
        ...
        if len(self.overlappedGenes) < 20 and len(self.overlappedGenes) > 0:
            ...
            if len(self.overlappedGenes) == 1:
                HTML += (
                    "<option value='{gene}' selected='selected'>{gene}</option>".format(
                        gene=self.overlappedGenes[0]
                    )
                )
            else:
    ...
    def getLengthsHTML(self):
        ...
        if getattr(self, "rgenID", False):
            ...
            default = (
                self.rgenRecord["DefaultGuideLength"]
                if "DefaultGuideLength" in self.rgenRecord
                else minLength
            )
        else:
            # get the first rgen record
            rgen = next(
                self.dbConnection.rgenCollection.find().sort([("rgenID", 1)]).limit(1)
            )
            # may need to do a next() call here
            minLength = rgen["MinGuideLength"]
            maxLength = rgen["MaxGuideLength"]
            default = (
                rgen["DefaultGuideLength"]
                if "DefaultGuideLength" in rgen
                else minLength
            )
        ...
        for i in range(int(minLength), int(maxLength) + 1):
            if str(i) == str(default):
                HTML += '<option value="{i}" selected>{i} (Default)</option>'.format(
                    i=str(i)
                )
            else:
    ...
    def getInputHTML(self):
        ...
        HTML = "<label for='searchInput'>Input Region ({length}bp):</label>".format(
            length=self.getRegionLength()
        )
        HTML += "<input type='text' class='form-control' required id='searchInput' onchange='updateInput()' onpaste='this.onchange();' value='{search}'>".format(
    ...
    def getOverlappingGenes(self):
        ...
        try:
            chm, start, end = re.search(
                r"(chr.+?):(\d+)\-(\d+)", self.searchInput
            ).groups()
        except Exception:
        ...
        overlappedGenes = []
        for gene in self.dbConnection.curr_geneCollection.find(
            searchQuery, {"Name": 1}
        ):
