...
class Config:
    def __init__(self, genome=None):
        ...
        self.setPaths()
        self.PRIMER3_SETTINGS = os.path.join(
            self.ROOT_PATH, "config/primer3settings.conf"
        )
        # where the primer3 in and output files are written
        self.PRIMER3_DIR = os.path.join(
            self.ROOT_PATH, "src/primer-design/files/primer3Files"
        )
        if self.config:
    ...
    def getChromSizes(self):
        ...
        try:
            with open(
                os.path.join(
                    self.ROOT_PATH, "jbrowse/data/" + self.genome, "seq/refSeqs.json"
                )
            ) as json_file:
    ...
    def fetchStrandofGene(ensid, genome):
        ...
        if record:
            ...
            raise ValueError(
                "ENSID:" + str(ensid) + " Does Not Exist in Current Gene Collection"
            )
...
def getCurrentGeneCollection(genome):
    ...
    if config:
        client = MongoClient(
            "mongodb://%s:%s@localhost" % (config["username"], config["password"])
        )
    else:
    ...
    for db in dbs:
        if db not in default_dbs:
            ...
            if metadataRecords.count() == 1:
                ...
                genomes.append(
                    (db, orgName, ensemblVersion)
                )  # store org code, name, and Ensembl release in list as tuple
            else:
...
def main():
    print(
        "A class for interfacing to the mongo database. Given a genome, connects to the correct database"
    )
    print(
        "and stores the database, release, current gene collection, guide collection, and primer collection."
    )
    print(
        "as attributes which other functions can access. Also stores the root path, i.e. where the web server"
    )
    print(
        "points to and the jbrowse, and primer design scripts are located. If the mongodb is secured"
    )
