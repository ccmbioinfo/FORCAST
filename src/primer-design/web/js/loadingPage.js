...
function checkRelease(){
	if (ENSMBL_RELEASE.toString() != RELEASE.toString()) {
		$('#warningMessage').append('Warning: JBrowse is using release ' + RELEASE + ', Ensembl is on release ' + ENSMBL_RELEASE);
	}
}
...
function PopupCenter(url, title, w, h) {
    ...
    var width = window.parent.innerWidth ? window.parent.innerWidth : document.documentElement.clientWidth ? document.documentElement.clientWidth : screen.width;
    var height = window.parent.innerHeight ? window.parent.innerHeight : document.documentElement.clientHeight ? document.documentElement.clientHeight : screen.height;
...
function blastPrimers(){
	$("#primerResultsTable").find("td").each(function () {	
	...
	$.ajax({
		...
		success: function(json){		
			...
			if (numAmplicons > 1){
				...
				preformattedString = '<br><button type="button" class="btn btn-link no-padding" data-toggle="modal" data-target="#'+rowID+'-qa-modal">'
				preformattedString += '<pre style="font-size:0.75rem;padding: 0.25rem;color: #b90e1e;text-decoration:underline;">';
				...
				productCell.append('<br><pre style="font-size:0.75rem;padding: 0.25rem;text-decoration: underline; color:red;">Error: No Amplicons</pre>');
			}	
		},
		error: function(xhr, status, err){
			// remove spinner
...
function createDiceyModal(rowID, amplicons, bindingSites){
	...
	html = `<div class="modal fade" id="`+rowID+`-qa-modal">
	...
	<ul class="nav nav-tabs" role="tablist">
  		<li class="nav-item">
    		<a class="nav-link active" id="amplicon-tab-`+rowID+`" data-toggle="tab" href="#ampliconsTab-`+rowID+`" role="tab" >Amplicons</a>
  		</li>
  		<li class="nav-item">
    		<a class="nav-link" id="bindingSite-tab-`+rowID+`" data-toggle="tab" href="#bindingSitesTab-`+rowID+`" role="tab">Binding Sites</a>
  		</li>
  	</ul>
	<div class="tab-content">
  	<div class="tab-pane fade show active" id="ampliconsTab-`+rowID+`" role="tabpanel" style="text-align:left; padding:1rem; margin:0;">`
	...
	for (var a in amplicons) {
		if (amplicons.hasOwnProperty(a)){
			...
			html += "Location: chr" + amplicons[a]['Chrom'] + ":" + amplicons[a]['ForPos'] + "-" + amplicons[a]['RevEnd'] + "<br>"
	...
	html +=`	
  	...
  	<div class="tab-pane fade" id="bindingSitesTab-`+rowID+`" role="tabpanel"><br>`
	...
	for (var b in bindingSites){
		if(bindingSites.hasOwnProperty(b)){
			...
			else{console.log("JSON object from dicey returned primer with unknown name");}
		}	
	}
	
	// create the tables to report the left/forward primer binding sites
	html += `
	<p style="float: left;">Forward Primer (`+forwardPrimer+`):</p>	
	...
	for (var b in forwardBindingSites) {
		...
		var strand = ''
		if (forwardBindingSites[b]['Ori'] == 'forward'){strand = "+";}
		else if (forwardBindingSites[b]['Ori'] == 'reverse'){strand = "-";}		
		
		//write the row
		html += "<tr>"
		html += "<td>chr"+forwardBindingSites[b]['Chrom']+":"+forwardBindingSites[b]['Pos']+"-"+forwardBindingSites[b]['End']+strand+"</td>"
	...
	html += `
	<p style="float: left;">Reverse Primer (`+reversePrimer+`):</p>	
	...
	for (var b in reverseBindingSites) {
		...
		var strand = ''
		if (reverseBindingSites[b]['Ori'] == 'forward'){strand = "+";}
		else if (reverseBindingSites[b]['Ori'] == 'reverse'){strand = "-";}	
		
		//write the row
		html += "<tr>"
		html += "<td>chr"+reverseBindingSites[b]['Chrom']+":"+reverseBindingSites[b]['Pos']+"-"+reverseBindingSites[b]['End']+strand+"</td>"
...
function displayHitResults(hitsObject, rowID){
	html = `<button type="button" class="btn btn-link no-padding" style="color: #b90e1e; text-decoration: underline;" data-toggle="modal" data-target="#`+rowID+`-hits-modal">
    		Multiple BLAST Hits
  		</button>
		<div class="modal fade" id="`+rowID+`-hits-modal">
		    <div class="modal-dialog modal-lg">
		      <div class="modal-content">
...
function updateGuideDownloadLinks() {
	...
	ids['genome'] = GENOME;
	count = 0;
	$.each($("#guideTable input:checked").closest("tr"),
		function () {
