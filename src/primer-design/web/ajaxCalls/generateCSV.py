    ...
    try:
        with open(filepath, mode="w") as csv_file:
            ...
            for g in guideResults:
                writer.writerow(
                    [str(g["guideLocation"]), str(g["guideSeq"]), str(g["pamSeq"])]
                )
            writer.writerow([])
            ...
            for record in primerResults:
                writer.writerow(
                    [
                        (str(geneName) + "_" + record["type"].lower() + "_F1"),
                        record["leftprimer"],
                        record["leftTM"],
                    ]
                )
                writer.writerow(
                    [
                        (str(geneName) + "_" + record["type"].lower() + "_R1"),
                        record["rightprimer"],
                        record["rightTM"],
                    ]
                )
...
def main():
    ...
    if genome:
        ...
        filename = writeCSV(
            genome, geneName, dbConnection.ROOT_PATH, guideResults, primerResults
        )
