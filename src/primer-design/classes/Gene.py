...
class Gene(object):
	...
	def __init__(self, geneName, genome, suppressWarnings=False, ENSID=None):
		...
		self.symbol, self.chromosome, self.gene_start, self.gene_end = self.getGeneCoordinates()
		self.exons = self.getExons()
		self.sortedExons = self.exons  # will be sorted in getSequenceAttributes
		self.sequence, self.sequenceLength, self.sequenceStart, self.sequenceEnd, self.strand = self.getSequenceAttributes()
	...
	def getExons(self):
		...
		transcripts = filter(lambda t: t['Parent'] == self.ensemblID and t['feature_type'] == 'transcript',
							 featureRequest.json())

		if not transcripts and not self.suppressWarnings:
			returnError("There were no valid protein-coding transcripts found for this gene in Ensembl")

		# get the ids of the filtered transcripts
		validTranscripts = [t['transcript_id'] for t in transcripts]
		exons = filter(lambda e: e['feature_type'] == 'exon' and e['Parent'] in validTranscripts, featureRequest.json())
		...
		for e in exons:
			if e['id'] not in completed:
				validExons.append(
					[e['start'], e['end'], e['strand'], e['ensembl_phase'], e['ensembl_end_phase'], e['exon_id']])
