...
class Primer3:
        ...
        for line in f:
            ...
            if file_match:
                ...
                if attemptNum in settingsDict:
                    settingsDict[attemptNum]["filepath"] = os.path.join(
                        settings_path_prefix, value
                    )
                else:
    ...
    def createPrimer3Input(
        self, primer, sequence, guideObject, targetSecondGuide=False
    ):
        """
        ...
        if primer == "WT":
            ...
            length = (
                abs(guideObject.wtTarget[1] - guideObject.wtTarget[0]) + 1
            )  # end of target - start of target
            if guideObject.emTarget is None:
                ...
                fileLines.append(
                    "SEQUENCE_TARGET=" + str(startTarget - 170) + "," + str(340)
                )
            else:
                fileLines.append(
                    "SEQUENCE_TARGET=" + str(startTarget) + "," + str(length)
                )
            fileLines.append("=")
            ...
            excisedSequence = (
                sequence[: guideObject.emTarget[0]]
                + sequence[guideObject.emTarget[1] :]
            )
            ...
            fileLines.append(
                "SEQUENCE_TARGET=" + str(startTarget - 149) + "," + str(300)
            )
            fileLines.append("=")
        ...
        filepath = os.path.join(
            self.Config.PRIMER3_DIR, f"{primer}_{guideObject.super.symbol}_input"
        )
        try:
    ...
    def attemptDesign(self, guideObject, primerType):
        ...
        while not foundPrimers and attemptNumber < numRetrys:
            ...
            if foundCount == 0:
                ...
                htmlResult += (
                    '<p id="retryText" class="text-danger">'
                    + primerType
                    + " Retry "
                    + str(attemptNumber)
                    + ": "
    ...
    def parsePrimers(self, primer3_output, primer, guideObject):
        ...
        for line in primer3_output.split("\n"):
            # perform searches
            sequenceSearch = re.search(
                "^PRIMER_(LEFT|RIGHT)_([0-9])_SEQUENCE=(.*)", line
            )
