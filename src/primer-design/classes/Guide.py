...
from Gene import Gene, returnError
...
class Guide(Gene):
    ...
    def guideQA(self):
        ...
        try:
            regulatoryRequest = requests.get(
                server + ext, headers={"Content-Type": "application/json"}, timeout=15
            )
        ...
        if len(filteredElements) > 0:
            ...
            for e in filteredElements:
                if e["feature_type"] == "transcript":
                    try:
                        ...
                        returnError(
                            "Problem fetching name of parent transcript" + str(e)
                        )
                else:
                    print(
                        "<tr><td>"
            ...
            print(
                '<span class="text-success"><i class="fa fa-check"></i><p style="display: inline;"> Guides passed QA check</p></span><br>'
            )
        ...
        if len(self.fileDictionary) > 0:
            if len(self.wtPrimers) > 0 and len(self.emPrimers) > 0:
                self.success = [
                    True,
                    True,
                ]  # both em and wt primers found & written successfully
    ...
    def writeGuideAPE(self):
        ...
        for key, apeString in self.fileDictionary.items():
            try:
                ...
                apeFile = open(
                    os.path.join(self.apeDir, self.super.genome, "features", filename),
                    "w",
                )
    ...
    def writeDownloads(self):
        ...
        defaultAPE = str(
            self.super.geneName + "_" + str(self.super.release) + "-automated_0_0.ape"
        )
        defaultCSV = str(
            self.super.geneName + "_" + str(self.super.release) + "-primers_0_0.csv"
        )
        genomeAPE_features = os.path.join(
            "../files/apeFiles", self.super.genome, "features"
        )
			...
			<div class="btn-group" role="group" style="float: left;">
				...
				<div class="btn-group" role="group">
					...
					<div class="dropdown-menu">
						<a class="dropdown-item" id="apeDownload" href="'''
            + os.path.join(genomeAPE_features, defaultAPE)
            + '''" download>Selected as APE</a>
						<a class="dropdown-item" id="csvDownload" href="'''
            + os.path.join(genomeCSV, defaultCSV)
            + """" download>Selected as CSV</a>
					</div>
				</div>
    ...
    def setTargets(self):
        ...
        if len(self.gRNAs) == 4:
            ...
            emTarget = None
            lengthExcise = abs(
                self.gRNAs[0][7] - self.gRNAs[0][8]
            )  # pretend the excised region is the guide itself
        else:
            returnError(
                str(len(self.gRNAs))
                + " Guides are marked as accepted for this gene. Can only design primers using 2, 3 or 4 guides"
            )
            exit()
    ...
    def checkOverlap(self):
        ...
        for i in range(0, len(self.gRNAs)):
            for j in range(i, len(self.gRNAs)):
                if (
                    self.gRNAs[i] != self.gRNAs[j]
                    and self.gRNAs[i][8] > self.gRNAs[j][7]
                ):
    ...
    def addCutsites(self):
        ...
        for g in self.gRNAs:
            ...
            try:
                ...
                try:
                    start = re.search(
                        guideSeq, self.super.sequence, re.IGNORECASE
                    ).start()
                    g.append(start)
                    g.append(start + len(guideSeq))
                    g.append("R")
                    ...
                    returnError(
                        "Could not find one of the guides in the sequence for the gene"
                    )
        ...
        for key, g in self.gRNADict.items():
            try:
                start = re.search(
                    g["guideSeq"], self.super.sequence, re.IGNORECASE
                ).start()
                g["start"] = start
                ...
                try:
                    start = re.search(
                        revSeq(g["guideSeq"]), self.super.sequence, re.IGNORECASE
                    ).start()
                    g["start"] = start
                    g["end"] = int(start) + len(g["guideSeq"])
                    g["direction"] = "R"
                    ...
                    returnError(
                        "Unable to find one of the guides in the sequence for this gene"
                    )
    ...
    def labelGuides(self):
        ...
        self.gRNADict = OrderedDict(
            sorted(self.gRNADict.items(), key=lambda x: int(x[1]["start"]))
        )
        ...
        if useManualLabel:
            ...
            for i, (key, guide) in enumerate(self.gRNADict.items()):
                if nGuides == 4:
                    ...
                    returnError(
                        "Problem labelling guides, the number of accepted guides must be 2, 3, or 4"
                    )
    ...
    def primer3Design(self):
        ...
        for primerType in ["WT", "EM"]:
            primerResult, retryString = primer3Connection.attemptDesign(
                self, primerType
            )
            if primerResult:
    ...
    def addWtFragments(self, emPrimers):
        for key, primer in emPrimers.items():
            if primer["genomicRightStart"] > primer["genomicLeftStart"]:
                ...
                wtFragementSize = (
                    int(primer["genomicRightStart"]) - int(primer["genomicLeftStart"])
                ) + 1
            else:
                wtFragementSize = (
                    int(primer["genomicLeftStart"]) - int(primer["genomicRightStart"])
                ) + 1
            primer["productSize"] = (
                str(primer["productSize"]) + "; " + str(wtFragementSize)
            )
    ...
    def addGenomicLocation(self, filteredPrimers):
        for key, primer in filteredPrimers.items():
            ...
            if self.super.strand == -1:
                ...
                primer["genomicLeftStart"] = (
                    int(self.super.sequenceEnd) - int(primer["leftstart"]) + 1
                )
                primer["genomicRightStart"] = (
                    int(self.super.sequenceEnd) - int(primer["rightstart"]) + 1
                )
            else:
                primer["genomicLeftStart"] = (
                    int(self.super.sequenceStart) + int(primer["leftstart"]) - 1
                )
                primer["genomicRightStart"] = (
                    int(self.super.sequenceStart) + int(primer["rightstart"]) - 1
                )
