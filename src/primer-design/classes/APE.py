...
class APE(object):
	...
	def __init__(self, geneObject, guideObject, mongoRecords):
		if not geneObject and not guideObject and not mongoRecords:
			raise ValueError('One of gene, guide or mongo record need to be defined when creating APE object')
		if geneObject:
			if guideObject or mongoRecords:
				raise ValueError('Only one of gene, guide or mongo record should be defined')
	...
	def labelGuides(self):
		...
		for i, guide in enumerate(self.guides):
			if nGuides == 4:
				...
				print("Error: Problem labelling guides, number of accepted guides must be 2, 3, or 4")	
	...
	def calculateDeletionSizes(self, ePrimer):
		...
		for i in range(0, len(self.guideObject.gRNAs)):
			for j in range(i, len(self.guideObject.gRNAs)):
				if i != j:
					guidePairCutSize = abs(self.guideObject.getCut(self.guideObject.gRNAs[i]) - self.guideObject.getCut(self.guideObject.gRNAs[j]))
					deletionStrings.append(str(self.guideObject.gRNAs[i][10]) + " - " + str(self.guideObject.gRNAs[j][10]) + " : " + str(fragmentSize - guidePairCutSize))
	...
	def calculateDeletionSizesMongo(self, ePrimer):
		...
		for i in range(0, len(self.guides)):
			for j in range(i, len(self.guides)):
				if i!=j:
					guidePairCutSize = abs(getCut(self.guides[i]) - getCut(self.guides[j]))
					deletionStrings.append(str(self.guides[i]['label']) + " - " + str(self.guides[j]['label']) + " : " + str(fragmentSize - guidePairCutSize))
	...
	def addPrimersMongo(self, fileToWrite, apeString, lastmatch):
		...
		for mongoPrimer in self.primers:
			...
			if geneDirection == -1:
				primerPrint['leftstart']= int(self.geneObject.sequenceEnd) - int(mongoPrimer['left_genomicLocation']) + 1
				primerPrint['rightstart'] = int(self.geneObject.sequenceEnd) - int(mongoPrimer['right_genomicLocation']) + 1
			
			else:
				primerPrint['leftstart'] = int(mongoPrimer['left_genomicLocation']) - int(self.geneObject.sequenceStart) + 1
				primerPrint['rightstart'] = int(mongoPrimer['right_genomicLocation']) - int(self.geneObject.sequenceStart) + 1
		...
		if len(self.primers) == 2:
			...
			if self.primers[0]['type'].lower() != self.primers[1]['type'].lower():
				...
				if self.primers[0]['leftprimer'].lower() == self.primers[1]['leftprimer'].lower():
					addComment += "\nCOMMENT     "
	...
	def splitAPE(self):
		apeString = '\n'.join(self.geneObject.APE)  # plain APE from the parent Gene object
		lastmatch = False
		# get the position of the last exon in the APE features (to write the guides after it)
		for match in re.finditer(
				r"exon\s*[0-9]*..[0-9]*\s*/label=\"[^\"]*\"\s*/ApEinfo_fwdcolor=\"[^\"]*\"\s*/ApEinfo_revcolor=\"[^\"]*\"\s*/ApEinfo_graphicformat=\"[^\"]*\"",apeString):
			lastmatch = match
		if not lastmatch:
			lastmatch = re.search(r"FEATURES             Location/Qualifiers", apeString)
			if not lastmatch:
		...
		if self.geneObject.exons:
			...
			for e in self.geneObject.exons:
				...
				if str(e_strand) == "-1":
					...
					filetowrite.append("     exon            " + str(self.geneObject.sequenceLength - relativeStart - exonLength + 1) + ".." + str(self.geneObject.sequenceLength - relativeStart))
				else:
					filetowrite.append("     exon            " + str(relativeStart + 1) + ".." + str(relativeStart + exonLength))
				filetowrite.append("                     /label=\"" + ensemblID + "\"")
				filetowrite.append("                     /ApEinfo_fwdcolor=\"#ffedc6\"")
				filetowrite.append("                     /ApEinfo_revcolor=\"#ffcc66\"")
				filetowrite.append("                     /ApEinfo_graphicformat=\"arrow_data {{0 1 2 0 0 -1} {} 0}")
