  ...
  for index, row in df.iterrows():
    for g in [str(x) for x in range(1,5)]:
      ...
      if not pd.isnull(row[gRNA_idx]):
        ...
        p = subprocess.Popen(["/usr/bin/python3.5","/var/www/html/ForCasT/src/guide-finder/GuideSearchAndScore.py", "--genome=GRCm39", "--gene="+str(gene), "--input="+str(location),"--output=output/"+str(gene)+".csv","--maxOffTargets=-1"], stdout=PIPE)
